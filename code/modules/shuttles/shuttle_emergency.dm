	...
	if (current_location == landmark_station)	//leaving the station
		spawn(0)
			...
			if (emergency_shuttle.evac)
				...
				priority_announcement.Announce(replacetext(replacetext(using_map.shuttle_leaving_dock, "%dock_name%", "[using_map.dock_name]"),  "%ETA%", "[estimated_time] minute\s"), "Transfer System", 'sound/AI/arfs/shuttle_depart.ogg') // ARFS Edit - Shuttle
	..()
